...
x = round(x);
y = round(y);
gaussian_weights = fspecial('Gaussian', [feature_width feature_width], 1);
...
for i = 1:size(x,1)
    ...
    Gmag_patch_flattened = Gmag_patch(:);
    Gdir_patch_flattened = Gdir_patch(:);
    Gdir_binned = zeros(36, 1);
    for m = 1:size(Gdir_patch_flattened)
        Gdir_bin_idx = min(floor((Gdir_patch_flattened(m) + 180)/10) + 1, 36);
        Gdir_binned(Gdir_bin_idx) = Gdir_binned(Gdir_bin_idx) + Gmag_patch_flattened(m) * gaussian_weights(m);
    end
    [~,max_bin_idx] = max(Gdir_binned);
    theta = (max_bin_idx * 10) - 5;
    ...
    for j = 1:feature_width/4:feature_width
        for k = 1:feature_width/4:feature_width
            ...
            Gdir_subpatch = mod(Gdir_subpatch(:) - theta, 360) - 180;
