...
if use_hard_negative_mining
    [bboxes, ~, image_ids] = run_detector(non_face_scn_path, w, b, feature_params, false);
    num_negative_examples = 5000;
    features_neg_hard = get_hard_negative_features(non_face_scn_path, bboxes, image_ids, feature_params, features_neg);
    X = [features_pos; features_neg_hard];
    Y = [ones(size(features_pos, 1), 1); -ones(size(features_neg_hard, 1), 1)];
    [w, b] = vl_svmtrain(X', Y, 0.001);
end
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b, feature_params, true);
