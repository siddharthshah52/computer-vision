...
function image_feats = get_bags_of_sifts_gaussian_pyramid(image_paths)
...
levels = 2;
image_feats = zeros(size(image_paths, 1), (levels+1)*vocab_size);
bin_size = 16;
...
%     img = vl_imsmooth(img, sqrt(bin_size/magnif)^2 - .25);
    for level=0:levels
        ...
        SIFT_features = datasample(SIFT_features, 1000, 2, 'Replace', true);
        pairwise_dist = vl_alldist2(vocab', single(SIFT_features)); % N x num sift features
        [~, I] = min(pairwise_dist);
        image_feats(i, level*vocab_size+1:(level+1)*vocab_size) = histcounts(I, 1:vocab_size+1)/(2^level);
