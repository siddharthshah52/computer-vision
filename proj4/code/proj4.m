...
% FEATURE = 'complementary features';
% FEATURE = 'gaussian pyramid';
% FEATURE = 'fisher';
...
%         if ~exist('vocab_ss.mat', 'file')
%             fprintf('No existing visual word vocabulary using self similarity descriptors found. Computing one from training images\n')
%             vocab_size = 200; %Larger values will work better (to a point) but be slower to compute
%             vocab = ssdesc(train_image_paths, vocab_size);
%             save('vocab_ss.mat', 'vocab')
%         end
        train_image_feats = additional_complementary_features(train_image_paths);
        test_image_feats = additional_complementary_features(test_image_paths);
        
    case 'gaussian pyramid'
        % YOU CODE build_vocabulary.m
        if ~exist('vocab.mat', 'file')
            fprintf('No existing visual word vocabulary found. Computing one from training images\n')
            vocab_size = 200; %Larger values will work better (to a point) but be slower to compute
        ...
        train_image_feats = get_bags_of_sifts_gaussian_pyramid(train_image_paths);
        test_image_feats  = get_bags_of_sifts_gaussian_pyramid(test_image_paths);
        
    case 'fisher'
        if ~exist('MEANS.mat', 'file') || ~exist('COVARIANCES.mat', 'file') || ~exist('PRIORS.mat', 'file') || ~exist('LL.mat', 'file') || ~exist('POSTERIORS.mat', 'file')
            fprintf('GMM vocabulary not found. Recomputing...')
            build_vocabulary_gmm(train_image_paths, vocab_size)
        end
