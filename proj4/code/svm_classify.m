...
train_samples = size(train_labels, 1);
lambda = 0.000005;
W = zeros(num_categories, size(test_image_feats, 2));
B = zeros(num_categories, 1);

for i=1:num_categories
    matching_indices = strcmp(categories{i}, train_labels);
    labels = -ones(train_samples, 1);
    labels(matching_indices) = 1;
    [w, b] = vl_svmtrain(train_image_feats', labels, lambda);
    W(i, :) = w;
    B(i) = b;
end
confidences = bsxfun(@plus, W * test_image_feats', B);
[~, I] = max(confidences);
predicted_categories = cell(size(test_image_feats, 1), 1);
for i=1:size(predicted_categories, 1)
    predicted_categories{i} = categories{I(i)};
end
end
