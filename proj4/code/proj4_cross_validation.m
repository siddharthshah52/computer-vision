...
iterations = [1 2 5 10];
iters = 10;
accuracies = zeros(iters, 1);
for iter=1:iters
    sample_train_image_paths = cell(1500, 1);
    sample_train_labels = cell(1500, 1);
    sample_test_image_paths = cell(1500, 1);
    sample_test_labels = cell(1500, 1);
    
    % Assuming that images from same category are grouped and that the
    % order in train and test is same
    for c=1:15
        [sample_train_image_paths_class, idx] = datasample(train_image_paths(((c-1)*100)+1:c*100), 100);
        sample_train_image_paths(((c-1)*100)+1:c*100) = sample_train_image_paths_class;
        sample_train_labels(((c-1)*100)+1:c*100) = train_labels(idx + (c-1)*100);
        
        [sample_test_image_paths_class, idx] = datasample(test_image_paths(((c-1)*100)+1:c*100), 100);
        sample_test_image_paths(((c-1)*100)+1:c*100) = sample_test_image_paths_class;
        sample_test_labels(((c-1)*100)+1:c*100) = test_labels(idx + (c-1)*100);
    end
...
% each function for more details.
    ...
    switch lower(FEATURE)    
        case 'tiny image'
            ...
            train_image_feats = get_tiny_images(sample_train_image_paths);
            test_image_feats  = get_tiny_images(sample_test_image_paths);
        ...
        case 'bag of sift'
            ...
            train_image_feats = get_bags_of_sifts(sample_train_image_paths);
            test_image_feats  = get_bags_of_sifts(sample_test_image_paths);
...
%     fprintf('Using %s classifier to predict test set categories\n', CLASSIFIER)
    ...
    switch lower(CLASSIFIER)    
        case 'nearest neighbor'
            ...
            predicted_categories = nearest_neighbor_classify(train_image_feats, sample_train_labels, test_image_feats);

        case 'support vector machine'
            % YOU CODE svm_classify.m 
            predicted_categories = svm_classify(train_image_feats, sample_train_labels, test_image_feats);

        case 'placeholder'
            %The placeholder classifier simply predicts a random category for
            %every test case
            random_permutation = randperm(length(sample_test_labels));
            predicted_categories = sample_test_labels(random_permutation); 
...
% confusions reasonable?confusion_matrix = zeros(num_categories, num_categories);
    num_categories = length(categories);
    confusion_matrix = zeros(num_categories, num_categories);
    for i=1:length(predicted_categories)
        row = find(strcmp(sample_test_labels{i}, categories));
    ...
    num_test_per_cat = length(sample_test_labels) / num_categories;
    confusion_matrix = confusion_matrix ./ num_test_per_cat;   
    accuracies(iter) = mean(diag(confusion_matrix));
    if any(iterations==iter)
        fprintf('Average accuracy over %d iterations is %s and standard deviation is %s \n', iter, mean(accuracies(1:iter)), std(accuracies(1:iter)))
    end
end
toc
