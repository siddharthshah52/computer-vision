...
%}
pairwise_dist = vl_alldist2(train_image_feats', test_image_feats'); % N x M matrix output
k = 20;
[~, indices] = sort(pairwise_dist);
indices = indices(1:k, :); % indices of the k nearest neighbors for each of M test examples
categories = {'Kitchen', 'Store', 'Bedroom', 'LivingRoom', 'Office', ...
       'Industrial', 'Suburb', 'InsideCity', 'TallBuilding', 'Street', ...
       'Highway', 'OpenCountry', 'Coast', 'Mountain', 'Forest'};
matched_indices = cell(size(categories, 2), 1);
for i=1:size(categories, 2)
    matched_indices{i} = strcmp(categories{i}, train_labels);
end
predicted_categories = cell(size(test_image_feats, 2), 1);
for i=1:size(indices, 2)
    max_sz = -1;
    category = '';
    for j=1:size(matched_indices)
        sz = nnz(matched_indices{j}(indices(:, i)));
        if sz > max_sz
            max_sz = sz;
